...
if [ "$simulator" = true ] || [ "$all_simulators" = true ]; then
  # Download to /vagrant/simulaters directory if needed
  if [ ! -e /vagrant/simulaters/$SIMULATOR_VERSION ]; then
     wget "https://build.coprhd.org/jenkins/userContent/simulators/smis-sim/1.0.0.0.1455598800/smis-simulators-1.0.0.0.1455598800.zip" -O "/vagrant/simulaters/$SIMULATOR_VERSION"
  fi
  # Install SMIS
  unzip /vagrant/simulaters/$SIMULATOR_VERSION -d /opt/storageos/
